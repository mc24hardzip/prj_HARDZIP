def process_address_and_format(df):
    ...
    def process_address(row):
        ...
        addr2 = " ".join([addr for addr in address1_list if
                          addr[-1] in classify1])
        if addr2: 
    ...
    df['manage_cost_inc'] = df['manage_cost_inc'].str.replace(',', ', ',
                                                              regex=True) 
...
def merge_dataframes(emd, sgg):
    ...
    """
    import pandas as pd
    combined = pd.merge(emd, sgg, left_on='sgg_cd',
                        right_on='sgg_cd_2', how='left')
...
def final_merge_and_clean(zigbang_emd, combined):
    ...
    import pandas as pd
    final = pd.merge(zigbang_emd, combined, left_on='emd_cd_2022',
                     right_on='emd_cd_2022_2', how='left')
    final = final.drop(columns=['sgg_cd_2', 'sido_nm_2',
                                'sgg_nm_2', 'emd_cd_2022_2'])
    return final
...
def add_distances_to_df(df, queries, API_KEY):
    ...
    for query, category_group_code, column_name in queries:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            params = [(row, query, category_group_code, API_KEY) for _, row in
                      df[["x_w84", "y_w84"]].iterrows()]
...
def calculate_ratios_and_refine_features(df):
    ...
    df['building_dandok_p'] = \
        (df['building_dandok'] / df['building_total']).round(3)
    df['building_apt_p'] = \
        (df['building_apt'] / df['building_total']).round(3)
    df['building_yeonlip_p'] = \
        (df['building_yeonlip'] / df['building_total']).round(3)
    df['building_dasedae_p'] = \
        (df['building_dasedae'] / df['building_total']).round(3)
    df['building_nonresidential_p'] = \
        (df['building_nonresidential'] / df['building_total']).round(3)
    df['building_others_p'] = \
        (df['building_others'] / df['building_total']).round(3)

    df['gender_ratio'] = \
        (df['ppltn_m'] / df['ppltn_f']).round(3)
    df['ppltn_foreign_domestic_ratio'] = \
        (df['ppltn_foreign'] / df['ppltn_domestic']).round(3)
    df['ppltn_upper_65_p'] = \
        (df['ppltn_upper_65'] / df['ppltn_total']).round(3)

    df['aging'] = df['ppltn_upper_65_p'].apply(
        lambda x: 1 if 0.07 < x < 0.14 else 0)
    df['aged'] = df['ppltn_upper_65_p'].apply(
        lambda x: 1 if 0.14 < x < 0.20 else 0)
    df['post_aged'] = df['ppltn_upper_65_p'].apply(
        lambda x: 1 if 0.20 < x <= 1 else 0)
    ...
    df['rent_adjusted'] = (df['rent'] + df['deposit'] * 0.05 / 12 +
                           df['manage_cost']).round(3)

    # 122 columns to 87 columns 
    column_include = ['id', 'service_type', 'address1', 'address2', '_floor',
                      'size_m2', 'sales_type', 'rent', 'deposit',
                      'manage_cost', 'manage_cost_inc', 'manage_cost_count',
                      'elevator', 'room_direction_text', 'images', 'parking',
                      'near_subways', 'near_subways_count', 'options',
                      'options_count', 'description', 'title', 'add1', 'add2',
                      'add3', 'supermarket_dist', 'convenience_store_dist',
                      'school_dist', 'subway_dist', 'cultural_venue_dist',
                      'public_institution_dist', 'hospital_dist', 'x_w84',
                      'y_w84', 'sgg_cd', 'emd_cd_2022', 'emd_cd_2020',
                      'sido_nm', 'sgg_nm', 'emd_nm', 'building_total',
                      'building_dandok_p', 'building_apt_p',
                      'building_yeonlip_p', 'building_dasedae_p',
                      'building_nonresidential_p', 'building_others_p',
                      'ppltn_total', 'gender_ratio',
                      'ppltn_foreign_domestic_ratio', 'ppltn_upper_65_p',
                      'aging', 'aged', 'post_aged', 'ppltn_dnsty',
                      'corp_to_pop', 'ppltn_adult_p', 'hhd_member_avg',
                      'hhd_total', 'hhd_collective_p', 'hhd_private_p',
                      'hhd_family_p', 'hhd_alone_p', 'tenure_total',
                      'tenure_self', 'tenure_jeonse', 'tenure_free',
                      'tenure_monthly', 'app_dasedae', 'app_dandok',
                      'app_nonresidential', 'app_apt', 'app_yeonlip',
                      'app_officetel', 'park_per_area', 'green_per_area',
                      'dust_level', 'ssg_ppltn', 'convenience_per_ppltn',
                      'shopping_per_ppltn', 'grocery_per_ppltn',
                      'restaurant_per_ppltn', 'medical_per_ppltn',
                      'culture_per_ppltn', 'gym_per_ppltn',
...
def preprocess_for_regression(df):
    ...
    """
    df["rent_adjusted"] = \
        df["deposit"] * 0.05 / 12 + df["rent"] + df["manage_cost"]
    ...
    df.drop(["manage_cost_inc", "near_subways", "options"],
            axis=1, inplace=True)
    ...
    df.drop(["tenure_self", "tenure_jeonse", "tenure_free", "tenure_monthly"],
            axis=1, inplace=True)
    ...
    df.drop(["deposit", "rent", "school_dist", "elevator", "manage_cost"],
            axis=1, inplace=True)
    return df