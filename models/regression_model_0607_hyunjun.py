...
def get_model():
    ...
    rf = RandomForestRegressor(
        n_estimators=621, min_samples_leaf=1, min_sampels_split=5
    )
...
def evaluate_models(scaling_method):
    X, y = get_xy(df)
    ...
    lr_train_rmse, lr_test_rmse, lr_r2_train, lr_r2_test = model_eval(
        lr, X_train_scaled, X_test_scaled, y_train, y_test
    )

    # 1-2) ridge
    ridge_train_rmse, ridge_test_rmse, ridge_r2_train, ridge_r2_test = model_eval(
        lr_ridge, X_train_scaled, X_test_scaled, y_train, y_test
    )

    # 1-3) lasso
    lasso_train_rmse, lasso_test_rmse, lasso_r2_train, lasso_r2_test = model_eval(
        lr_lasso, X_train_scaled, X_test_scaled, y_train, y_test
    )
    ...
    (
        ...
        elastic_r2_train,
        elastic_r2_test,
    ) = model_eval(elastic, X_train_scaled, X_test_scaled, y_train, y_test)

    # 1-5) rf
    rf_train_rmse, rf_test_rmse, rf_r2_train, rf_r2_test = model_eval(
        rf, X_train_scaled, X_test_scaled, y_train, y_test
    )

    # 1-6) gb
    gb_train_rmse, gb_test_rmse, gb_r2_train, gb_r2_test = model_eval(
        gb, X_train_scaled, X_test_scaled, y_train, y_test
    )

    # 1-7) xgb
    xgb_train_rmse, xgb_test_rmse, xgb_r2_train, xgb_r2_test = model_eval(
        xgb, X_train_scaled, X_test_scaled, y_train, y_test
    )

    # 1-8) lgmb
    lgb_train_rmse, lgb_test_rmse, lgb_r2_train, lgb_r2_test = model_eval(
        lgbm, X_train_scaled, X_test_scaled, y_train, y_test
    )
    ...
    (
        ...
        lr_poly_r2_train,
        lr_poly_r2_test,
    ) = model_eval(lr, X_train_scaled, X_test_scaled, y_train, y_test)

    # 2-2) ridge
    (
        ridge_poly_train_rmse,
        ridge_poly_test_rmse,
        ridge_poly_r2_train,
        ridge_poly_r2_test,
    ) = model_eval(lr_ridge, X_train_scaled, X_test_scaled, y_train, y_test)

    # 2-3) lasso
    (
        lasso_poly_train_rmse,
        lasso_poly_test_rmse,
        lasso_poly_r2_train,
        lasso_poly_r2_test,
    ) = model_eval(lr_lasso, X_train_scaled, X_test_scaled, y_train, y_test)

    # 2-4) elastic
    (
        elastic_poly_train_rmse,
        elastic_poly_test_rmse,
        elastic_poly_r2_train,
        elastic_poly_r2_test,
    ) = model_eval(elastic, X_train_scaled, X_test_scaled, y_train, y_test)
    ...
    (
        ...
        rf_poly_r2_train,
        rf_poly_r2_test,
    ) = model_eval(rf, X_train_scaled, X_test_scaled, y_train, y_test)
    ...
    (
        ...
        gb_poly_r2_train,
        gb_poly_r2_test,
    ) = model_eval(gb, X_train_scaled, X_test_scaled, y_train, y_test)
    ...
    (
        ...
        xgb_poly_r2_train,
        xgb_poly_r2_test,
    ) = model_eval(xgb, X_train_scaled, X_test_scaled, y_train, y_test)
    ...
    (
        ...
        lgb_poly_r2_train,
        lgb_poly_r2_test,
    ) = model_eval(lgbm, X_train_scaled, X_test_scaled, y_train, y_test)
